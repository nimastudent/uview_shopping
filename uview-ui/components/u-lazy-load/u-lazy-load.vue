	...
	<view class="u-wrap" :style="{
	 ...
	 :class="'u-lazy-item-' + elIndex">
		<view :class="'u-lazy-item-' + elIndex">
			<image :style="{borderRadius: borderRadius + 'upx', height: imgHeight}" v-if="!isError" class="u-lazy-item"
			 :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded" @error="loadError" @tap="clickImg"></image>
			<image :style="{borderRadius: borderRadius + 'upx', height: imgHeight}" class="u-lazy-item error" v-else :src="errorImg"
...
<script>
	 ...
	 * @property {String} threshold 触发加载时的位置，见上方说明，单位 upx（默认300）
	 ...
	 * @property {String Number} border-radius 图片圆角值，单位upx（默认0）
