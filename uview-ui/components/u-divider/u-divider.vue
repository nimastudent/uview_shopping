<template>
	<view class="u-divider" :style="{
		height: height == 'auto' ? 'auto' : height + 'upx',
 ...
 * @property {String Number} half-width 文字左或右边线条宽度，数值或百分比，数值时单位为upx
 * @property {String} border-color 线条颜色，优先级高于type（默认#dcdfe6）
 * @property {String} color 文字颜色（默认#909399）
 * @property {String Number} fontSize 字体大小，单位upx（默认26）
 * @property {String} bg-color 整个divider的背景颜色（默认呢#ffffff）
 * @property {String Number} height 整个divider的高度，单位upx（默认40）
 * @property {String} type 将线条设置主题色（默认primary）
 * @property {Boolean} useSlot 是否使用slot传入内容，如果不传入，中间不会有空隙（默认true）
 * @property {String Number} margin-top 与前一个组件的距离，单位upx（默认0）
 * @property {String Number} margin-bottom 与后一个组件的距离，单位upx（0）
...
export default {
	name: 'u-divider',
	props: {
		// 单一边divider横线的宽度(数值)，单位upx。或者百分比
		halfWidth: {
			type: [Number, String],
			default: 150
		},
	...
	computed: {
		lineStyle() {
			...
			else style.width = this.halfWidth + 'upx';
